...
**`create_app()` is a factory with injectable dependencies** (`app/main.py`): `create_app(*, engine=None, transcriber=None, shutdown_hook=None)`. It stores `engine`, `transcriber`, `settings`, and `shutdown_hook` on `app.state`; the default transcriber (real WhisperX) is constructed lazily **only when none is injected**. On startup it also marks any recording left in `transcribing` (orphaned when a previous process was killed/restarted mid-job) as `failed` so the UI can offer retry (`repo.fail_orphaned_transcriptions`). Tests inject `FakeTranscriber` and a no-op `shutdown_hook` via `tests/conftest.py`'s `client` fixture, so production ML and real signals never fire under pytest. Routes/`/health`/`/shutdown` are registered before the static `web/dist` mount so the SPA mount never shadows the API.
