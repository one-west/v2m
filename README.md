...
[브라우저] 녹음(MediaRecorder, webm/opus)
  ...
  → SQLite 저장 → 화면에 화자별 전사본 표시
  → "전사본 + 프롬프트 복사 후 claude.ai 열기" → 붙여넣기 → 정형화 회의록
  (전사본 Markdown/TXT 내보내기 가능)
```

## 상태

- ✅ **백엔드**(`server/`, FastAPI): 업로드 → 백그라운드 전사 잡 → 전사본/프롬프트/내보내기 API. 39 테스트.
- ✅ **프론트엔드**(`web/`, Vite + React + TS): 녹음·목록·전사본 뷰·claude.ai 복사·내보내기. 23 테스트.
...
앱은 LLM을 호출하지 않습니다. 전사본 화면에서 **"전사본 + 프롬프트 복사 후 claude.ai 열기"**를 누르면
한국어 회의록 지시 프롬프트와 화자별 전사본이 클립보드에 복사되고 claude.ai가 열립니다 — 붙여넣으면
요약·핵심 논의·결정사항·액션아이템으로 정형화됩니다. (긴 회의는 입력 한도 안내가 표시됩니다.)
